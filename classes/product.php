...
<?php
	class product
	{
		...
		public function show_productbyCat($idLoai)
		{
			...
			if (isset($_GET['size'])){
				$sizeSP = $_GET['size'];
				$query = "SELECT * FROM tbl_sanpham, tbl_loaisanpham WHERE tbl_sanpham.maLoai = tbl_loaisanpham.maLoai AND tbl_sanpham.maLoai = '$idLoai' AND tbl_sanpham.trangThaiSanPham = '1' AND sotap = '$sizeSP' ORDER BY maSanPham ASC LIMIT $tungTrang, $sanPhamTungTrang";
			}
			...
			if (isset($_POST['sortby']) && !empty($_POST['sortby'])){
				...
				if (isset($_GET['size'])){
					...
					if ($sortby == 'Cao thấp'){
						$query = "SELECT * FROM tbl_sanpham, tbl_loaisanpham WHERE tbl_sanpham.maLoai = tbl_loaisanpham.maLoai AND tbl_sanpham.maLoai = '$idLoai' AND tbl_sanpham.trangThaiSanPham = '1' AND sotap = '$sizeSP' ORDER BY giaSanPham DESC LIMIT $tungTrang, $sanPhamTungTrang";
					}else if ($sortby == 'Thấp cao'){
						$query = "SELECT * FROM tbl_sanpham, tbl_loaisanpham WHERE tbl_sanpham.maLoai = tbl_loaisanpham.maLoai AND tbl_sanpham.maLoai = '$idLoai' AND tbl_sanpham.trangThaiSanPham = '1' AND sotap = '$sizeSP' ORDER BY giaSanPham ASC LIMIT $tungTrang, $sanPhamTungTrang";
					}
		...
		public function getAllProductbyCat($idLoai) //Dùng cho phân trang
		{
			...
			if (isset($_GET['size'])){
				$sizeSP = $_GET['size'];
				$query = "SELECT * FROM tbl_sanpham, tbl_loaisanpham WHERE tbl_sanpham.maLoai = tbl_loaisanpham.maLoai AND tbl_sanpham.maLoai = '$idLoai' AND tbl_sanpham.trangThaiSanPham = '1' AND sotap = '$sizeSP' ORDER BY maSanPham ASC ";
			}
		...
		// 	$sotap = mysqli_real_escape_string($this->db->link, $data['sotap']);
		...
		// 	if ($tenSanPham == "" || $maLoai == "" || $sotap == "" || $mieuTaSanPham == ""|| $giaSanPham == "" || $soLuongSanPham == ""){
